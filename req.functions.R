# A set of functions used to deine the capacity ranges for the technology/case attributes

req.range <- function(x, lower=-Inf, upper=Inf){
  # a uniform function being 1 for lower>x>upper, else 0
  ifelse(x>=lower & x<=upper, 1, 0)
  }

req.trapez <- function(x, a, b=(d-a)/2+a, c=b, d){
